...
import { Unlock, BookOpen, Sparkles, ArrowRight, UserCheck } from "lucide-react";
...
function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    setFetching(true);
    supabase.from("topic_sets").select("*, cards(count)").order("order_index").then(({ data }) => {
      setSets(data || []);
      setFetching(false);
    });
  ...
  const totalCards = sets.reduce((s, x) => s + (x.cards?.[0]?.count ?? 0), 0);
